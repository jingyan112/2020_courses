...
const register_user_service = async(email, password, hash, verification) => {
  ...
  const res1 = await executeQuery("SELECT * FROM small_app_users WHERE email = $1", email);
  if (res1 && res1.rowCount > 0) {
    data.logs.push('This email is already registrated, you can login directly!');
    return data;
  }
  
  const res2 = await executeQuery("INSERT INTO small_app_users (email, password) VALUES ($1, $2);", email, hash);
